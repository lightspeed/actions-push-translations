...
const pushTranslations = ({ slackWebhookUrl }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const resources = flat_1.default(nestedResources, { maxDepth: 2 });
    ...
    for (const resourceName of resourceNames) {
        ...
        const diffSummary = yield git.diffSummary(["HEAD~1"]);
        ...
        const { stdout } = yield execAsync(`tx push -s -r ${resourceName}`);
        console.log(`Transifex push output:\n${stdout}`);
        // Track changes in resource files for Slack notification
        const diff = yield git.diff(["HEAD~1", "--", sourceFilename]);
        const changes = diff
            .split("\n")
